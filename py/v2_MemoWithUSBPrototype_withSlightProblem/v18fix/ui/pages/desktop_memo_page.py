...
class FixedEditorSurface(ttk.Frame):
    ...
    def _toggle_reminder(self, row_idx: int) -> None:
        ...
        self.pull_state_to_widgets()
        self.page.on_rows_changed([row_idx], force_full=True, reason="toggle_reminder")
    ...
    def _commit_task_text(self, row_idx: int, reason: str = "manual") -> None:
        ...
        self.page.on_rows_changed([row_idx], force_full=True, reason="task_text")
    ...
    def _set_time_value(self, row_idx: int, value: str) -> None:
        ...
        self.pull_state_to_widgets()
        self.page.on_rows_changed([row_idx], force_full=True, reason="set_time")
...
class DesktopMemoPage(ttk.Frame):
    ...
    def on_rows_changed(self, local_rows: list[int], force_full: bool = False, reason: str = "edit") -> None:
        ...
        if force_full:
            ...
            self._append_log("事项/铃铛/时间修改提升为整块列表区域局部刷新，以减小闪烁。")
            self._append_log(f"排队列表区域局刷 region=({region.x},{region.y},{region.width},{region.height}) page={self.current_page + 1}/{self.total_pages}")
            self._queue_region(region)
            return

        if before_page != self.current_page or before_total != self.total_pages:
            region = PartialRegion(x=LIST_RECT[0], y=LIST_RECT[1], width=LIST_RECT[2] - LIST_RECT[0], height=LIST_RECT[3] - LIST_RECT[1])
            self._append_log("页码或总页数变化，提升为整块列表区域局刷。")
        elif not changed_rows:
            region = self._region_for_rows(local_rows)
        else:
            region = self._region_for_rows(changed_rows)
        ...
        try:
            ...
            def worker() -> None:
                try:
                    ...
                    tb = traceback.format_exc()
                    self.after(0, lambda: self._on_flash_error(exc, tb))
