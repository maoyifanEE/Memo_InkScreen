...
bool renderFullBw(const uint8_t* data, size_t len) {
  ...
  // Prime the controller base map for later partial updates.
  // The vendor driver explicitly requires SetRAMValue_BaseMap before stable partial refresh.
  EPD_HW_Init();
...
bool renderPartialBw(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint8_t* data, size_t len) {
  ...
  // Use the panel vendor partial API directly. Its argument order is (height, width).
  EPD_Dis_Part(x, y, partialRegionBuffer, h, w);
